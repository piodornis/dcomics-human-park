...
O=Path('outputs/modulia-raumstudie-v2'); O.mkdir(exist_ok=True)
W,L,H=4.8,5.6,3.0
model={'status':'PROPOSAL / EXPLORATION','units':'m','room':{'east_west':W,'north_south':L,'height':H},'axes':'x nach Ost/Fenster, y nach Süd, z nach oben; lokale Planrichtungen','door':{'wall':'West','y':[2.25,3.35],'height':2.3},'window':{'wall':'Ost','y':[1.05,4.55],'z':[1.0,2.55]},'console':{'y':[2.0,4.8],'back_x':.90,'front_x':1.84,'back_bow':.10,'front_inset':.22,'height':.88},'cameras':{}}
faces=[]; objects=[]
def face(v,c,tag=''):
...
wall_x(0,0,2.25,0,H,'#e9dcc6');wall_x(0,3.35,L,0,H,'#e9dcc6');wall_x(0,2.25,3.35,2.3,H,'#e9dcc6')
...
box('Türlaibung Nord',-.22,2.20,0,.23,.05,2.35,'#526e70');box('Türlaibung Süd',-.22,3.35,0,.23,.05,2.35,'#526e70');box('Türsturz',-.22,2.20,2.3,.23,1.2,.05,'#526e70')
# An open junction outside: left when leaving means south (+y).
for j in range(6):face([[-1.5,2.15+j*.55,0],[0,2.15+j*.55,0],[0,2.70+j*.55,0],[-1.5,2.70+j*.55,0]],'#d4c3a6','Vorraum')
wall_x(-1.5,2.15,2.25,0,2.5,'#dbc6a8');wall_x(-1.5,3.35,5.45,0,2.5,'#dbc6a8')
box('Außentür Nord',-1.55,2.20,0,.1,.07,2.3,'#6d7b72');box('Außentür Süd',-1.55,3.33,0,.1,.07,2.3,'#6d7b72');box('Außentür Sturz',-1.55,2.20,2.3,.1,1.20,.06,'#6d7b72')
box('Felsen seitlich im Vorraum',-1.30,1.74,0,.62,.39,1.8,'#bf8662')
...
cams={'A':{'eye':[3.25,5.40,1.62],'target':[2.35,.15,1.35],'fov':72},'B':{'eye':[3.05,.82,1.62],'target':[2.70,5.5,1.35],'fov':76},'C':{'eye':[4.68,2.8,1.62],'target':[.2,2.8,1.35],'fov':78},'D':{'eye':[-.12,2.8,1.62],'target':[4.8,2.8,1.42],'fov':74}}
model['cameras']=cams
...
plan+=rect(0,0,W,L,'#ede1cb')+rect(-1.5,2.2,1.5,3.3,'#e5d5bc')
for ob in objects:
...
x,y=xy(0,2.25);plan+=f'<path d="M{x},{y} v{1.1*scale}" stroke="#faf5eb" stroke-width="8"/>'
...
plan+='<text x="210" y="40" font-size="24">Modulia · gemeinsames Raumgerüst v2</text><text x="210" y="65" font-size="15">PROPOSAL · Tür mittig: 2,25–3,35 m · Raum 4,80 × 5,60 m</text><text x="340" y="118" font-size="14">Archiv / Nord</text><text x="655" y="325" font-size="15">Fenster / Ost</text><text x="40" y="520" font-size="14">Gang nach links</text><text x="40" y="541" font-size="13">beim Hinausgehen ↓</text><text x="65" y="280" font-size="13">Außentür ←</text><text x="700" y="170" font-size="14">K1: Eckschrank</text><text x="700" y="555" font-size="14">K2: Fensterschrank</text><text x="210" y="650" font-size="15">P1 Tisch · P2 hohe Ablage · P3 oben am Archiv</text><text x="210" y="675" font-size="15">P4 Fensterbank · P5 niedriger Fensterschrank</text><text x="210" y="715" font-size="14">Maße und Kameras sind Vorschläge, keine Bestandsvermessung.</text></g></svg>'
plan=plan.replace('</g></svg>','<text x="220" y="310" font-size="13">Türmitte 2,80 m</text></g></svg>')
...
head='<!doctype html><html lang="de"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Modulia – Raumstudie v2</title><style>body{font:16px system-ui;background:#faf5eb;color:#294954;margin:24px auto;max-width:1100px;padding:0 16px}button{font:inherit;padding:10px 20px;border:1px solid #59747a;border-radius:5px;background:#fff8e9;color:#294954;cursor:pointer}button[aria-pressed=true]{background:#294954;color:white}nav{display:flex;gap:8px;flex-wrap:wrap}section svg{width:100%;max-height:72vh}small{line-height:1.6}h1{font-size:26px}p{max-width:850px}</style><h1>Modulia · Raumstudie v2</h1><p>PROPOSAL / EXPLORATION · Ein gemeinsames Modell für vier Ansichten. Farben kennzeichnen Bauteile; Pflanzen und Felsen sind Platzhalter. Keine neue Stilentscheidung.</p><nav>'
